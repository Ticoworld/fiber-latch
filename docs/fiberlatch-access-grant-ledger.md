...
| -- | -------------------- | ---------------- | -------------------------------- | -------------------- | -------- | ------------------------ | ----------- |
| D1 | FiberLatch Access scope and design | Existing backend docs, repository evidence, and the historical proof | `docs/fiberlatch-access-scope.md` and any later design notes that freeze the scope boundary | DRAFTED FOR REVIEW | Working-tree scope document created on branch `grant/week-1-2-scope-freeze`; not yet committed or reviewed | Review and approve the scope freeze | Confirm the scope document against the approved grant boundary |
